...
- [x] 5.2 全局快捷键：注册 `Cmd+Shift+T`（或自定义），随时呼出/隐藏主窗口（类似 iTerm2 的热键窗）
- [x] 5.3 内置编辑器联动（Save & Upload）：
  - 右键远程文件 → "在编辑器中打开" → `sftpDownload` 至系统临时目录 → `openPath` 调用系统默认编辑器
  - 每 2 秒轮询 `localStat` 的 modified 时间，检测到变更后弹窗"文件已修改，是否上传？"
  - 确认后自动 `sftpUpload` 覆盖远程文件
